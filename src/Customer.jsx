...
const Customer = () => {
  ...
  const fetchChefs = async () => {
    try {
      const response = await axios.get('https://localhost:7242/api/Customer/foodlovers');
  ...
  const handleDelete = async (chefId) => {
    try {
      await axios.delete(`https://localhost:7242/api/Customer/foodlover/${chefId}`);

      toast.success("Foodlover deleted successfully!");

      setChefs(prevChefs => prevChefs.filter(chef => chef.id !== chefId));
    } catch (error) {
      toast.error("Failed to delete foodlover.");
